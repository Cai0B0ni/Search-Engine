...
public class TestDoubleLinkedList extends BaseTest {
    ...
    public void testDeleteMiddleElement() {
        ...
        list.add("e");
        list.delete(3);
        this.assertListValidAndMatches(new String[]{"a", "b", "c", "e"}, list);
        list.delete(1);
        this.assertListValidAndMatches(new String[]{"a", "c", "e"}, list);

    }

    //Expected Behavior 2: assert that after deleting an item indexOf no
    // longer can find that item in the list
    @Test(timeout = 1 * SECOND)
    ...
    //Expected Behavior 3: assert that after deleting an item, the size
    // is properly updated
    @Test(timeout = 1 * SECOND)
    ...
    // Edge Case 1: assert that after deleting the very first item in
    // the list, it is removed from the list
    @Test(timeout = 1 * SECOND)
    public void testDeleteFrontElement() {
        ...
        list.delete(0);
        assertEquals(1, list.size());
        this.assertListValidAndMatches(new String[]{"c"}, list);
    ...
    // Edge Case 2: assert that after deleting the very last item in the
    ...
    // Edge Case 3: assert that after deleting multiple copies of the same item,
    // all occurrences have been correctly removed
    @Test(timeout = 1 * SECOND)
    public void testDeleteDuplicates() {
        ...
        list.add(2);
        list.delete(4);
        this.assertListValidAndMatches(new Integer[]{2, 3, 1, 2}, list);

        list.delete(3);
        this.assertListValidAndMatches(new Integer[]{2, 3, 1}, list);

        list.delete(0);
        this.assertListValidAndMatches(new Integer[]{3, 1}, list);

        assertTrue(!list.contains(2));
    }

    // Empty case: assert that after deleting the only item in a list the
    // list is now empty
    @Test(timeout = 1 * SECOND)
    ...
    // Invalid Input: assert that if delete is passed an index that is not valid it
    ...
    @Test(timeout = 1 * SECOND)
    public void testIndexOfAndDelete() {
        IList<String> list = this.makeBasicList();
